...
# Define path to your fallback file inside the package in case of GEO download problem
fallback_file <- system.file("inst/extdata", "gse_fallback.rds", package = "beadarray")
gse <- tryCatch({
    # Attempt the live download
    GEOquery::getGEO("GSE33126", returnType = "ExpressionSet")[[1]]
}, error = function(e) {
    message("Notice: GEO download failed or timed out. Using local fallback data.")
    
    if (file.exists(fallback_file)) {
        readRDS(fallback_file)
    } else {
        stop("Fallback data file not found: ", e$message)
    }
})
